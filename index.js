...
const clickHandler = (e, c) => {
    console.log(e === questions.right, e, questions[c].right)
    ...
    currentQues++
    if (currentQues + 1 <= questions.length) countDiv.innerHTML = currentQues + 1 + ' из ' + questions.length
...
const render = (c) => {
    ...
    questions[c].answers.forEach((i, index) => {
        ...
        console.log(item)
        item.addEventListener('click', () => clickHandler(index, c), {once: true})
...
function end() {
    ...
    if (startDate > 59) {
        timeOfTest = Math.floor(startDate/60) + ' минут, ' + startDate % 60 + ' секунд'
    } else {
...
function tick() {
    ...
    timerInterval = setInterval(() => {
        i++;
        if (i > 59) {
            timer.innerHTML = 'Время теста: ' + Math.floor(i/60) + ' минут, ' + i % 60 + ' секунд'
        } else {
